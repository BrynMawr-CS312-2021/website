...
* You can use a _sorting shortcut_ which takes advantage of the fact that particles do not shift too much between frames (called _spatial and temporal consistency_). In this approach, go through the list once, compare consecutive particles, and swap if one particle should be in front of another. 
